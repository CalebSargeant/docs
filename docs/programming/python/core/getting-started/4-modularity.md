...
- `if __name__ == '__main__':` if name = main, execute function, if name != main, it knows it's being imported into another module, not executed. By using this statement, the python script can be executed as a script or module.
- `__name__` will be set as `__main__` depending on how it's used - if running `python words.py`, `__main__` will return as `__name__` instead.
