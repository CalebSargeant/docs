...
Notice where the spaces are being removed. `lstrip` removes space from
right-hand-side (leading), `rstrip` removes space from left-hand-side
(trailing), and `strip` from both sides (not the middle).
