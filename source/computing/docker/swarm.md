...
-   Raft database created to store root CA, configs and secrets
    ...
    > -   Replicates logs amongst Managers via mutual TLS in "contrl
    >     plane"

## Overlay Multi-Host Networking

-   Just choose [\--driver overlay]{.title-ref} when creating network
...
-   [docker network create \--driver overlay mydrupal]{.title-ref}

-   [docker service create \--name psql \--network mydrupal -e
    POSTGRES_PASSWORD=mypass postgres]{.title-ref}

-   [docker service create \--name dripal \--network mydrupal -p 80:80
    drupal]{.title-ref}
...
-   [docker service create \--name elasticsearch \--replicas 3 -p
    9200:9200 elasticsearch:2]{.title-ref}
...
-   We use [docker stack deploy]{.title-ref} rather than docker service
    create
-   Stack manages all those objects for us, including overlay network
    per stack. Adds stack name to start of their name
-   New [deploy:]{.title-ref} key in Compose file. Cant do
    [build:]{.title-ref}
-   Compose now ignores [deploy:]{.title-ref}, Swarm ignores
    [build:]{.title-ref}
-   [docker-compose]{.title-ref} cli not needed on Swarm server

![image](_images/swarm-stack.png)

-   [docker stack deploy -c example-voting-app-stack.yml
    voteapp]{.title-ref}
-   [docker stack serices voteapp]{.title-ref}
-   [docker stack ps voteapp]{.title-ref}
...
-   Default is Managers and Workers "control plan" is TLS + Mutual
    Auth
...
-   [/run/secrets/secret_name]{.title-ref} or
    [/run/secrets/secret_alias]{.title-ref}

-   Local docker-compose can use file-based secrets, but not secure

### Secrets with Services

-   [docker secret create psql_user psql_user.txt]{.title-ref}
-   [echo "myDBPassword" \| docker secret create psql_pass
    -]{.title-ref}
-   [docker secret ls]{.title-ref}
-   [docker secret inspect psql_user]{.title-ref}
-   [docker service create \--name psql \--secret psql_user \--secret
    psql_pass -e POSTGRES_PASSWORD_FILE=/run/secrets/psql_pass -e
    POSTGRES_USER_FILE=/run/secrets/psql_user postgres]{.title-ref}
-   [docker service update \--secret-rm]{.title-ref}

### Secrets with Stacks

-   [docker service create \--name search \--replicas 3 -p 9200:9200
    elasticsearch:2]{.title-ref}
-   [docker stack deploy -c docker-compoes.yml mydb]{.title-ref}

## Swarm Lifecycle

-   [docker-compose exec psql cat /run/secrets/psql_user]{.title-ref}

### Full App Lifecycle with Compose

-   Single set of Compose files for:
-   Local [docker-compose up]{.title-ref} development environemnt
-   Remote [docker-compose up]{.title-ref} CI environment
-   Remote [docker stack deploy]{.title-ref} production environment
-   Note: [docker-compose -f a.yml -f b.yml config]{.title-ref} mostly
    works
-   Note: Compose [extends:]{.title-ref} doesnt work yet in Stacks
...
-   Also has scale & rollback subcommand for quicker access
    ...
    > -   [docker service scale web=4]{.title-ref} and [docker service
    >     rollback web]{.title-ref}
...
-   Just update the image used to a newer version
    ...
    >     <servicename>]{.title-ref}

-   Adding an environment variable and remove a port

    > -   [docker service update \--env-add NODE_ENV=production
    >     \--publish-rm 8080]{.title-ref}

> -   Change number of replicas of two services
>     > -   [docker service scale web=8 api=6]{.title-ref}

### Swarm Updates in Stack FIles

Same command, just edit the YAML file, then

[docker stack deploy -c file.yml <stackname>]{.title-ref}

## Healthchecks

-   [HEALTHCHECK]{.title-ref} was added in 1.12

-   Supported in Dckerfile, Compose YAML, docker run, and Swarm Services

-   Docker engine will [exec]{.title-ref}'s the command in the
    container

    > -   e.g curl localhost

-   it expects [exit 0]{.title-ref} (OK) or [exit 1]{.title-ref} (Error)
...
-   Healthcheck status shows up in [docker container ls]{.title-ref}

-   Check last 5 healthchecks with [docker container
    inspect]{.title-ref}
...
-   Options for healthcheck command
    ...
    > -   [\--interval=DURATION (default: 30s)]{.title-ref}
    > -   [\--timeout=DURATION (default: 30s)]{.title-ref}
    > -   [\--start-period=DURATION (default: 0s) (17.09+)]{.title-ref}
    > -   [\--retries=N (default:3)]{.title-ref}

-   Basic command using default options

    > -   [HEALTHCHECK curl -f http://localhost/ \|\| false]{.title-ref}

-   Custom options with the command

    > -   [HEALTHCHECK \--timeout=2s \--interval=3s \--retries=3 CMD
    >     curl -f http://localhost/ \|\| exit 1]{.title-ref}
