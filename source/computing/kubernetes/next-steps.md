...
-   Avoid stateful workloads for fist few deployments until you're good
    at the basics
...
-   All our [kubectl]{.title-ref} commands just talk to the Kubernets
    API
-   Kubernetes has limited built-in templating, versioning, tracking,
    and management of your apps
-   There are now over 60 3rd party tools to do that , but many are
    defunct
...
-   [docker app]{.title-ref} and compose-on-kubernetes are Docker's way
...
-   Some distributions have their own GUI (Rancher, Docker Ent,
    OpenShift)
...
-   There are some built-in, to hide system stuff from
    [kubectl]{.title-ref} "users"

    > -   [kubectl get namespaces]{.title-ref}
    > -   [kubectl get all \--all-namespaces]{.title-ref}

-   Context changes [kubectl]{.title-ref} cluster and namespace

-   See \~/.kube/config file

-   [kubectl config get-contexts]{.title-ref}

-   [kubectl config set\*]{.title-ref}
...
-   Kubernetes has become "differencing and scheduling engin backbone"
    for so many new projects
-   Knative - Serverless workloads on Kubernetes
-   k3s - mini, simple Kubernetes
-   k3OS - Minimal Linux OS for k3s
-   Service Mesh - New layer in distributed app traffic for better
