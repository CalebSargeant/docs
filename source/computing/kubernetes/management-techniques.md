...
-   Every resource in kubernetes has a specification or "spec"
    ...
    > -   [kubectl create deployment sample \--image nginx \--dry-run -o
    >     yaml]{.title-ref}

-   You can output those templates with [\--dry-run -o yaml]{.title-ref}
...
-   Using dry-run with yaml output we can see the generators
    ...
    > -   [kubectl create deployment test \--image nginx \--dry-run -o
    >     yaml]{.title-ref}
    > -   [kubectl create job test \--image nginx \--dry-run -o
    >     yaml]{.title-ref}
    > -   [kubectl expose deployment/test \--port 80 \--dry-run -o
    >     yaml]{.title-ref}
...
-   The goal is to reduce its features to only create pods
    ...
    > -   Right now its defaults to creating Deployments (with the
    >     warning)
    > -   It has lots of generators but they are all deprecated
    > -   The idea is to make it easy like [docker run]{.title-ref} for
    >     one-off tasks
...
-   Using dry-run we can see which generators are used
    ...
    > -   [kubectl run test \--image nginx \--dry-run]{.title-ref}
...
-   Imperative: I boil water, scoop out 42 grams of medium-fine grounds,
    pour over 700 grams of water, etc.
-   Declarative: "Barista, I'd like a cup of coffee (barista is the
    engine that works through the steps, including retrying to make a
    cup and is only finished when I have a cup)"

### Kubernetes Imperative

-   Examples: [kubectl run]{.title-ref}, [kubectl create
    deployment]{.title-ref}, [kubectl update]{.title-ref}
...
-   Example: [kubectl apple -f my-resources.yaml]{.title-ref}
...
-   More work than [kubectl run]{.title-ref} for just starting a pod
...
-   Most important rule
    ...
    > -   Learn the Imperative CLI for easy control of local and test
    >     setups
    > -   Move to apply -f file.yml and apply -f directoryfor prod
    > -   Store yaml in git, git commit each change before
    > -   THis trains you later doing GitOPs (where git commits are
