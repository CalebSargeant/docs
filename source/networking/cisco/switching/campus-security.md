...
-   VLAN Hopping: Hacker negotiates a trunk connection with a switch,
    moves between vlans
...
1.  Disable CDP wherever possible (`no cdp enable`)
2.  Lock down spanning-tree (`switchport host` and `bpduguard`)
3.  disable trunk negotiation on access ports (`sw mo acc`)
4.  Physical security is key
5.  Place unused ports in a black-hole vlan (`lin vty 0 4`,
    `trans in ssh`)
6.  Use SSH when possible
