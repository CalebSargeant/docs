...
The below adds the radius-server to the switch with the secret key
"test" and enables authentication via radius, then local
...
The below configuration is applied on the Core Switch(es) in to block
the guest network (VLAN100) from accessing the rest of the networks.
