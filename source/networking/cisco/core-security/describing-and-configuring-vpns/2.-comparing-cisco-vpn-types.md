...
-   It is best practice to have the most secure algorithm with the
    highest priority, while the least secure algorithm will be the
    lowest priority
...
-   Allows tunnel interface to create Multiple VPNs with multiple
    devices
-   Used with DMVPN
...
-   Client that is installed on user's device that allows a remote
    access VPN
-   Can connect to IOS, ASA, or FTD
-   Can have different profiles push out different parameters IP
    address, ACLs, login times
